...
export default function PricingView({ onNavigate, density = 'compact' }: PricingViewProps) {
  ...
  const handleOpenCheckout = async (planId: string) => {
    ...
    if (!priceId) {
      setCheckoutError('This plan is not available for checkout right now. Please contact support.');
      return;
    }
      ...
      <AnimatePresence>
        {checkoutError && (
          <motion.div
            ...
            animate={{ opacity: 1, x: 0, y: 0 }}
            exit={{ opacity: 0, x: 30 }}
            className="fixed bottom-6 right-6 z-50 bg-red-600 text-white p-3.5 rounded-xl shadow-2xl border border-red-700 text-xs flex items-start gap-3 max-w-sm"
          >
            <AlertCircle className="w-4.5 h-4.5 shrink-0 mt-0.5" />
            <div className="flex-1">
              <span className="font-mono text-[9px] text-red-100 font-bold uppercase tracking-wider block mb-0.5">Checkout Error</span>
              <p className="font-medium">{checkoutError}</p>
            </div>
            <button
              onClick={() => setCheckoutError(null)}
              className="text-red-100 hover:text-white shrink-0"
              aria-label="Dismiss"
            >
              ✕
            </button>
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  disabled={isLoading}
                  className={`w-full font-sans text-xs font-bold py-3.5 rounded-xl transition-all active:scale-98 cursor-pointer flex items-center justify-center gap-1.5 disabled:opacity-60 disabled:cursor-not-allowed ${
                    isPopular 
